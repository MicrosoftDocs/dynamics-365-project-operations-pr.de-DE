...
## <a name="add-custom-time-entries-for-your-own-use"></a><a name="add"></a>Hinzufügen benutzerdefinierter Zeiteinträge für Ihren eigenen Gebrauch.

Zeiteintragn sind eine Kernentität, die für mehrere Szenarien verwendet werden kann. In der April-Welle 1 2020 wurde die TESA-Kernlösung eingeführt, die eine **Einstellungen**-Entität und eine neue **Zeiteintragsbenutzer**-Sicherheitsrolle bietet. Die neuen Felder **msdyn_start** und **msdyn_end**, die eine direkte Beziehung zu **msdyn_duration** haben, wurden ebenfalls eingeführt. Die neue Entität, Sicherheitsrolle und die Felder ermöglichen einen einheitlicheren Zeitansatz für mehrere Produkte.


### <a name="time-source-entity"></a>Zeitquellentität
| Feld | Beschreibung des Dataflows | 
|-------|------------|
| Name des Dataflows  | Der Name der Zeitquelleneingabe, der bei der Erstellung des Zeiteintrags als Auswahlwert verwendet wird. |
| Standardzeitquelle [Zeitquelle: isdefault] | Standardmäßig darf nur eine Zeitquelle als Standardzeitquelle markiert sein. Mit dieser Funktion können Eingaben standardmäßig auf eine Zeitquelle zurückgesetzt werden, wenn keine angegeben ist. |
|Zeitquellentyp [Zeitquelle: sourcetype] | Der Quelltyp ist eine Option (Zeiteintragsquellentyp), mit der die Zeitquelle einer App zugeordnet werden kann. Zusätzliche Werte werden diesem Optionssatz hinzugefügt, wenn zusätzliche Anwendungen hinzugefügt werden. Beachten Sie, dass Microsoft Werte reserviert, die größer als 190.000.000 sind.|
...
- Zeiteinträge, die nur mit **msdyn_date** und **msdyn_duration** angegeben erstellt werden, starten um Mitternacht und **msdyn_start** und **msdyn_end** werden entsprechend aktualisiert.
...
- Fügen Sie das benutzerdefinierte Feld dem Schnellerfassungs-Dialogfeld hinzu.
- Konfigurieren Sie das Raster, um das benutzerdefinierte Feld anzuzeigen.
- Fügen Sie das benutzerdefinierte Feld entweder dem Aufgabenfluss „Zeile bearbeiten“ oder dem Aufgabenfluss „Zelle bearbeiten“ hinzu.
...
Sie müssen das benutzerdefinierte Feld dem Dialogfeld **Schnellerfassung für Zeiteintrag** hinzufügen. Benutzer können dann einen Wert eingeben, wenn sie Zeiteinträgen hinzufügen, indem sie **Neu** auswählen.
...
Sie können die Ansicht **Meine wöchentlichen Zeiteinträge** anpassen und ihr das benutzerdefinierte Feld hinzufügen. Sie können die Position und Größe des benutzerdefinierten Felds im Raster auswählen, indem Sie diese Eigenschaften in der Ansicht bearbeiten.
...
4. Konfigurieren Sie die Parameter für das Raster für den wöchentlichen Zeiteintrag. Legen Sie das Feld **Startdatum** auf **msdyn_date**, das Feld **Dauer** auf **msdyn_duration** und das Feld **Status** auf **msdyn_entrystatus** fest. 
5. Für die standardmäßige Ansicht wird das Feld **Schreibgeschützte Statusliste** auf **192350002,192350003,192350004**, das Feld **Aufgabenflow zur Zeilenbearbeitung** auf **msdyn_timeentryrowedit** und das Feld **Aufgabenflow zur Zellenbearbeitung** auf **msdyn_timeentryedit** festgelegt. 
6. Sie können diese Felder anpassen, um den schreibgeschützten Status hinzuzufügen oder zu entfernen, oder eine andere aufgabenbasierte Umgebung (task-based experience, TBX) für die Zeilen- oder Zellenbearbeitung zu verwenden. Diese Felder sollten an einen statischen Wert gebunden werden.
